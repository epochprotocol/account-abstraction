...
contract EpochRegistry is IEpochRegistry {
    ...
    function verifyTransaction(
        ...
        if (task.isBatchTransaction) {
            ...
            require(
                keccak256(abi.encode(task.destinations)) ==
        ...
        if (task.timeConditionId != 0) {
            ExecutionWindow memory timeCondition = executionWindowMapping[
                task.timeConditionId
            ];
            require(
            ...
            OnChainCondition memory onChainCondition = onChainConditionMapping[
                task.onChainConditionId
            ];
            bool _onChainConditionStatus = _checkOnChainCondition(
                onChainCondition
            );
            require(
                _onChainConditionStatus,
                "Registry: On-chain Condition Failed"
            );
        }
    ...
    function _checkOnChainCondition(
        OnChainCondition memory onChainCondition
    ) internal returns (bool) {
        (bool success, bytes memory response) = onChainCondition
            .dataSource
            .call(onChainCondition.encodedQuery);
        require(success, "Registry: Invalid OnChainCondition");
        return
            onChainCondition.conditionChecker.checkCondition(
                onChainCondition.encodedCondition,
                response
            );
    }
        ...
        returns (bool _send, address _dest, uint256 _value, bytes memory _func)
    {
        require(taskStatus[taskId] == false, "Registry: Task already executed");
        require(
            taskMapping[taskId].taskId == taskId,
            "Registry: Task does not exist"
        );
        ...
        _value = value;
        //updated taskID here
        ExecutionWindow memory executionWindow = executionWindowMapping[
            task.timeConditionId
        ];
        if (executionWindow.recurring) {
            executionWindow.executionWindowStart += executionWindow
                .recurrenceGap;
    ...
    function _fetchData(
        ...
        (bool status, bytes memory response) = dataSource.dataSource.call(
            dataSource.encodedQuery
        );
        require(status, "Registry: data fetch failed");
        bytes32 dataToOverwrite = response.getFixedData(
            dataSource.dataPosition
        );
        bytes memory overwrittenData = _func.overwriteStaticDataWithSignature(
            dataToOverwrite,
            dataSource.positionInCallData
        );
